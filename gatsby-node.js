...
exports.createPages = async ({ graphql, actions, reporter }) => {
    ...
    const getTranslations = postData => {
        return Object.values(postData).map(trans => {
            const { path: urlPath, lang } = trans.frontmatter;
            return {
                link: `/${lang}/${urlPath}`,
                lang
            };
    ...
    const createPosts = (postsList, lang) => {
        if (postsList.length > 0) {
            postsList.forEach((post, index) => {
                ...
                const nextPostId = index === postsList.length - 1 ? null : postsList[index + 1].id;

                const translations = getTranslations(postsByDirectoryName[post.fields.directoryName]);

                const { path: urlPath, lang: postLang } = post.frontmatter;
                const pagePath = `/${lang}/${urlPath}`;
